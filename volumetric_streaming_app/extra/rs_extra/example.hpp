...
void upload_from_raw_data(const char* frame_data, const  unsigned& _format, const int& _width, const int& _height, const unsigned& __stream_type, const unsigned& __stream_index)
    {
        if (!_gl_handle)
            glGenTextures(1, &_gl_handle);
        ...
        auto width = _width;
        auto height = _height;
        _stream_type = (rs2_stream)__stream_type;
        _stream_index = __stream_index;
        ...
        case RS2_FORMAT_RGBA8:
            ...
            break;
        case RS2_FORMAT_Y8:
            glTexImage2D(GL_TEXTURE_2D, 0, GL_RGB, width, height, 0, GL_LUMINANCE, GL_UNSIGNED_BYTE, frame_data);
            break;
        case RS2_FORMAT_Y10BPACK:
            glTexImage2D(GL_TEXTURE_2D, 0, GL_LUMINANCE, width, height, 0, GL_LUMINANCE, GL_UNSIGNED_SHORT, frame_data);
            break;
        default:
