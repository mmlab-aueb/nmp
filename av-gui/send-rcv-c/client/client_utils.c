...
int initialize_values_of_libwebsocket_state(
    ...
    *ws_context = lws_create_context(ws_ctx_info);
    ...
    ws_client_info->host     = lws_canonical_hostname(*ws_context);
    ws_client_info->origin   = address;
    ws_client_info->protocol = protocols[0].name;
...
char** parse_for_gstreamer_args(void *argtable[]) {
    ...
    /* Format port and ssl into temporary buffers */
    char port_buf[16], ssl_buf[4];
    snprintf(port_buf, sizeof(port_buf), "%d", port_arg->ival[0]);
    snprintf(ssl_buf,  sizeof(ssl_buf),  "%d", ssl_arg->ival[0]);

    /* strdup each string so they live past this function */
    char *client_str      = strdup("client");
    char *port_str        = strdup(port_buf);
    char *ssl_str         = strdup(ssl_buf);
    char *address_str     = strdup(address_arg->sval[0]);
    char *path_str        = strdup(path_arg->sval[0]);

    /* Allocate argv array on the heap (+1 for NULL) */
