...
#include "message_handling.h"      // Contains process_incoming_message() etc.
#include "signaling_context.h"     // Contains create_signaling_context(), add_client(), etc.
#include "message_protocol.h"      // Contains create_json_message(), parse_json_message(), etc.

/* Global variables for graceful shutdown and global state */
static volatile int interrupted = 0;
static struct signaling_context *global_signaling_context = NULL;
static struct signaling_state   *global_signaling_state = NULL;
 ...
 */
static int callback_signaling(struct lws *wsi,
                              ...
                              size_t len)
{
    ...
    case LWS_CALLBACK_ESTABLISHED:
        ...
        /* Add this client to the global signaling context */
    ...
    case LWS_CALLBACK_RECEIVE:
        /*
         * Assume that the incoming message fits in the per-session buffer.
         * (Your code uses a 4096-byte buffer.)
         */
...
int main(int argc, char **argv)
{
    ...
    info.port = 9000;       /* Choose your desired port */
    ...
    lwsl_user("[Signaling] Server started on port %d\n", info.port);
