...
# python3 janusvideoroom-publish.py --server=wss://jitsi.aueb.gr:10000 1234
import random
import ssl
import websockets
import asyncio
...
v4l2src ! videoconvert ! tee name=videotee \
    videotee. ! queue ! vp8enc deadline=1 ! rtpvp8pay ! queue ! application/x-rtp,media=video,encoding-name=VP8,payload=97 ! sendrecv. \
    videotee. ! queue ! autovideosink \
alsasrc ! audioconvert ! audioresample ! queue ! opusenc ! rtpopuspay ! queue ! application/x-rtp,media=audio,encoding-name=OPUS,payload=96 ! sendrecv.
'''
