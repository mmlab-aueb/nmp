...
# python3 janusvideoroom-subscribe.py --server=wss://jitsi.aueb.gr:10000 1234
# remember to change the "feed"  number below
import random
import ssl
import websockets
import asyncio
...
VIDEO_SRC="videotestsrc pattern=snow"
# VIDEO_SRC="v4l2src"
...
PIPELINE_DESC = '''
 ...
 {} ! {} !  queue ! application/x-rtp,media=video,encoding-name={},payload=96 ! recvonly.
'''.format(VIDEO_SRC, encoder, payloader, rtp_encoding)
...
#    async def send_ice_candidate_message(self, _, mlineindex, candidate):
#        icemsg = {
#            "candidate": candidate,
#            "sdpMLineIndex": mlineindex
#        }
#        print("Sending ICE Candidate:", icemsg)
#        await self.signaling.sendtrickle(icemsg)
...
#        print("Incoming stream linked")
    ...
    def debug_negotiation_needed(self, webrtcbin):
        print("iiiiiiiiiiiiii333333333333333333333")
            
    def debug_ice_candidate(self, webrtcbin, mlineindex, candidate):
        print("kkkkkkkkkkkkkk333333333333333333333")
        print(f"'on-ice-candidate' signal triggered.")
        print(f"mlineindex: {mlineindex}, candidate: {candidate}")
        
    def on_pad_added(self, webrtcbin):
        #print(f"Pad added callback triggered: {pad.get_name()}")
        #if pad.get_direction() == Gst.PadDirection.SRC:
        print("oooooooooooo333333333333333333333")  # To confirm the callback is triggered
        #else:
        #    print("Pad direction is not SRC.")


    def send_ice_candidate(self, webrtcbin, mlineindex, candidate):
        print("333333333333333333333")  # To confirm the callback is triggered
        if candidate:
            print(f"333333333333333333333: ICE Candidate Generated - {candidate}")
            icemsg = {'candidate': candidate, 'sdpMLineIndex': mlineindex}
            print(f"Sending ICE: {icemsg}")
            loop = asyncio.new_event_loop()
            loop.run_until_complete(self.signaling.sendtrickle(icemsg))
        else:
            print("333333333333333333333: No ICE candidate generated")
    ...
    def start_pipeline(self):
        ...
        signals = [signal for signal in dir(self.webrtc) if 'signal' in signal.lower()]
        print("Available signals:", signals)
        #self.webrtc.connect('pad-added', self.on_pad_added)
        #self.webrtc.connect('on-negotiation-needed', self.send_ice_candidate)
        #self.webrtc.connect('on-negotiation-needed', self.debug_negotiation_needed)
        #self.webrtc.connect('pad-added', self.on_incoming_stream)
        #self.webrtc.connect('on-ice-candidate', self.debug_ice_candidate) 
        #self.webrtc.connect('on-ice-candidate', lambda webrtcbin, mlineindex, candidate: print(f"Candidate: {candidate}"))
...
#        request = {"request": "start", "room": self.room_id}
#        jsep = {"sdp": text, "trickle": True, "type": "answer"}
#
#        # Send the SDP answer
#        loop = asyncio.new_event_loop()
#        loop.run_until_complete(self.signaling.sendmessage(request, jsep))
    ...
    def on_answer_created(self, promise, _, __):
        ...
        print("Setting local SDP description...")
        local_promise = Gst.Promise.new()
        self.webrtc.emit('set-local-description', answer, local_promise)
        local_promise.interrupt()
        
        # Prepare the "start" request
        self.request = {"request": "start", "room": 1234}  # Replace with your room ID
        self.jsep = {"sdp": text, "trickle": True, "type": "answer"}

        # Send the SDP answer to Janus
        loop = asyncio.new_event_loop()
        loop.run_until_complete(self.signaling.sendmessage(self.request, self.jsep))

    async def handle_sdp(self, msg):
        print (msg)
        if 'sdp' in msg:
            sdp = msg['sdp']
            assert msg['type'] == 'offer', "Expected an SDP offer"
    ...
    async def loop(self):
        ...
        joinmessage = { "request": "join", "ptype": "subscriber", "room": 1234, "streams": [{"feed": 3039024936554497}] }
        
        await signaling.sendmessage(participants)
